...
class draw_simulation:
    ...
    def experiment1(n=100, p=1000):
        ...
        e = np.random.normal(0, 0.05, n + 20).reshape(
            n + 20,
        )
        Y = F + e
    ...
    def experiment2(
        n, ntest, p, start, end, corrxz, corrzz, r2, block_corr=False, lin=False
    ):
        """
        ...
        if lin:
            ...
            ztest = +(
                np.tan(x1test)
                + np.tan(x2test)
                + np.sin(2 * np.pi * x1test)
                + np.sin(2 * np.pi * x2test)
        ...
        x1test = x1test.reshape(
            ntest,
        )
        x2test = x2test.reshape(
            ntest,
        )
        ...
        if block_corr:
            ...
            Sigma = np.block(
                [
                    [Sigma1, np.zeros((length_group1, length_group2))],
        ...
        Z = norm.cdf(Z)
        Ztest = (
            np.random.multivariate_normal(np.zeros(p), var_xtest * Covmat, ntest)
            + Xtest @ P
        )
        ...
        X = np.row_stack(
            (np.column_stack((x1, x2, Z)), np.column_stack((x1test, x2test, Ztest)))
        )
...
class train:
    ...
    def get_minibatch(
        y, X, l, s, sigma, reg, minibatch, kern, sampling_strat, nn_fraction
    ):
        n, p = np.shape(X)
        select = l != 0
        ...
        if sampling_strat != "unif":
            ...
            dist, samples = train.get_NN(
                y, X[:, select], X[i, select], l[select], s, kern, minibatch, nn_fraction
            )
        else:
    ...
    def get_gradients_gp_ss(
        y_sample, X_sample, K, Ktild, l, s, sigma, v, c, lmbda, minibatch, grad_kern, n
    ):
        p = len(l)
        ...
        for i in range(p):
            ...
            grad_logL_l[i] = (
                0.5 * np.sum(A * g)
                - v * l[i] * (c * lmbda[i] + (1 - lmbda[i])) * minibatch / n
            )
    ...
    def get_step_size(
        ...
        if optimisation == "adam":
            ...
            step_size = (
                learn_rate
                * (sum_grads / (1 - beta**t))
                / ((sum_sq_grads / (1 - beta2**t)) ** 0.5 + eps)
            )
    ...
    def kernel_param_optimise(
        ...
        y_sample, X_sample, K, Ktild = train.get_minibatch(
            y, X, l, s, sigma, reg, minibatch, kern, sampling_strat, nn_fraction
        )
        ...
        while t < maxiter:
            ...
            # Getting gradients
            grad_logL_l, grad_logL_s, grad_logL_sig = train.get_gradients_gp_ss(
                y_sample,
                X_sample,
                K,
                Ktild,
            ...
            # Getting step sizes
            step_size_l, sum_sq_grads_l, sum_grads_l = train.get_step_size(
                grad_logL_l,
                sum_sq_grads_l,
                sum_grads_l,
                beta,
            ...
            step_size_s, sum_sq_grads_s, sum_grads_s = train.get_step_size(
                grad_logL_s,
                sum_sq_grads_s,
                sum_grads_s,
                beta,
            ...
            step_size_sig, sum_sq_grads_sig, sum_grads_sig = train.get_step_size(
                grad_logL_sig,
                sum_sq_grads_sig,
                sum_grads_sig,
                beta,
            ...
            if store_elbo:
                logLs[t - 1] = model_funcs.logL(y_sample, Ktild) * n / minibatch - (
                    0.5
                    * np.sum(v * (c * lmbda + (1 - lmbda)) * l**2 - lmbda * np.log(c))
                    + p / 2 * np.log(v)
                ) * (v != c)
    ...
    def get_lmbda(l, v, c, logpi, log1_pi):
        return 1 / (
            1 + (1 / c) ** 0.5 * np.exp(-0.5 * l**2 * v * (1 - c) + log1_pi - logpi)
        )
        ...
        while i < aCAVI_iter:
            ...
            if max_iter > 0:
                ...
                (
                    ...
                    beta2=Beta2,
                    eps=eps,
                    optimisation_sums=optimisation_ssgs,
                    v=v,
                    c=c,
                    lmbda=lmbda[select],
                    maxiter=max_iter,
                    print_=print_gradsteps,
                    kern=kern,
                    grad_kern=grad_kern,
                    store_ls=store_ls,
                    L=L,
                    iters=i,
                    store_elbo=store_elbo,
                )
            else:
                logLs = np.zeros(1)
            ...
            if store_elbo:
                ...
                neg_kl_gamma = np.sum(
                    reg_lmbda * (logpi - np.log(reg_lmbda))
                    + (1 - reg_lmbda) * (log1_pi - np.log(1 - reg_lmbda))
                )
                neg_kl_pi = (
                    (a - ahat) * logpi
                    + (b - bhat) * log1_pi
                    + sp.gammaln(ahat)
                    + sp.gammaln(bhat)
                    - sp.gammaln(ahat + bhat)
                )
                neg_kl_theta = 0.5 * np.sum(lmbda[select == False]) * np.log(
                    c
                ) + 0.5 * np.sum(select == False) * np.log(v)
                Elbos = np.append(Elbos, logLs + neg_kl_gamma + neg_kl_pi + neg_kl_theta)

            # Optimising q(gamma),q(pi)
            lmbda[select + latest_select_out] = train.get_lmbda(
                l[select + latest_select_out], v, c, logpi, log1_pi
            )
            ...
            if prune:
                ...
                select = selectnew
                optimisation_ssgs[0], optimisation_ssgs[1] = (
                    optimisation_ssgs[0][selectnew_subset],
                    optimisation_ssgs[1][selectnew_subset],
                )

            # Printing updates
            i += 1

            # Code error: Elbo is undefined.
            # if print_aCAVI:
    ...
    def model(
        ...
        X,
        hyper_arg=["v"],
        hyper_vals=[10**4 * 2 ** np.linspace(np.log2(1000), -np.log2(1000), 11)],
        training_args=[],
        training_arg_vals=[],
    ):
        """
        ...
        if training_args:
            for j in range(len(training_args)):
                index = (
                    np.where(training_args[j] == np.array(training_master_args))[0][0] - 2
                )
        ...
        for i in range(len(hyper_vals[0])):
            if selections > 0:
                ...
                for j in range(len(hyper_arg)):
                    hyper_index = np.where(
                        hyper_arg[j] == np.array(training_master_args)
                    )[0][0]
...
class evaluation:
    def Burkner_LOOLPD(y, X, results, kern, reg, regvar, post_var):
        ...
        l = results[0][select]
        Xselect = X[:, select]
        Kxx = kernel_funcs.ARD(l, results[1], Xselect, kern) + (
            results[2] ** 2 + reg + regvar
        ) * np.eye(n)
            ...
            if q > 0:
                ...
                if NN < n:
                    ...
                    for i in range(n):
                        ...
                        Ktest = kernel_funcs.ARDtest(
                            l=lpred, s=s, X=Xselect[nn], Xtest=Xi, kern=kern
                        )
                        ...
                        logpredictives[j] += (
                            -0.5 * np.log(ypostvar[i])
                            - 0.5 * (ypostmean[i] - yi) ** 2 / ypostvar[i]
                        )

                    logpredictives[j] += -n / 2 * np.log(2 * np.pi)

                else:
                    logpred, ypostmean, ypostvar = evaluation.Burkner_LOOLPD(
                        y, X, results, kernel_funcs.gaussian, reg, reg_var, post_var
                    )
...
class test:
    def posterior_predictive(
        ...
        if post_var:
            ypost_var = (
                Ktesttest
                - Ktest @ np.linalg.solve(Ktild, Ktest.T)
                + np.diag(np.ones(len(Ktesttest))) * (reg + sig**2)
            )
            if latents:
                fpost_var = (
                    Ktesttest
                    - Ktest @ np.linalg.solve(K + np.diag(np.ones(len(K))) * reg, Ktest.T)
                    + np.diag(np.ones(len(Ktesttest))) * reg
                )

        return fpost_mean, fpost_var, ypost_mean, ypost_var

    def posterior_predictive_nn(
        y,
        X,
        Xtest,
        l,
        ...
        for i in range(m):
            ...
            if use_tree:
                ...
                nn = train.get_NN(
                    y, Xsearch, Xtestsearch[i], 1, s, kern, NN, fraction=fraction
                )[1]
    ...
    def model(y, X, Xtest, testing_algorithm, Results, weights, MC_samples=100):
        ...
        for i in range(len(weights)):
            if sampled_weights[i] > 0:
                ...
                fm, fv, ym, yv = testing_algorithm(
                    y=y,
                    X=X[:, select].reshape(n, q),
                    Xtest=Xtest[:, select].reshape(m, q),
                    l=l[select],
                    s=s,
                    sig=sig,
                    post_var=False,
